package com.example.lib_common.web.common;
...
public class X5WebChromeClient extends com.tencent.smtt.sdk.WebChromeClient {
    ...
    public void onHideCustomView() {
        if (mXCustomView == null)// 不是全屏播放状态
            return;
        getmIWebPageView().getActivity().setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_UNSPECIFIED);
    ...
    public void onProgressChanged(com.tencent.smtt.sdk.WebView view, int newProgress) {
        ...
        if (newProgress == 100 && mIWebPageView != null) {
            //加载完成
...
//        Activity activity = getmIWebPageView().getActivity();
//        if (activity instanceof DefWebViewActivity && ((DefWebViewActivity) activity).mTopBar != null) {
//            ((DefWebViewActivity) activity).mTopBar.setTitle(title);
//        }

        if (mGetWebViewTitle != null) {
            mGetWebViewTitle.getTitle(title);
        }
    }
    ...
    private getWebViewTitle mGetWebViewTitle;

    public void GetWebViewTitle(getWebViewTitle getWebViewTitle) {
        mGetWebViewTitle = getWebViewTitle;
    }
