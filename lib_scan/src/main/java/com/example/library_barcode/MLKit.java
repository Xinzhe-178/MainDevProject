...
import com.example.library_barcode.barcodescanner.BarcodeScannerProcessor;
import com.example.library_barcode.hardware.BeepManager;
...
public class MLKit implements LifecycleObserver {
     ...
     * 自加
     *
     * @param supportMultiple 是否支持一次识别多个码
     *                        现逻辑为: 只有在默认通过相机扫描时, 才支持
     *                        通过相册识别时: 不支持, 因为会有问题(不可说)
     */
